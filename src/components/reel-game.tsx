...
function GarudaMaskTile({ isHit }: { isHit?: boolean }) {
  return (
    <div
      className={`relative size-full rounded-xl border-[2.5px] border-[#fbbf24] bg-gradient-to-b from-[#b45309] via-[#451a03] to-[#1a0801] p-1 shadow-[inset_0_2px_4px_rgba(255,255,255,0.7),0_4px_10px_rgba(0,0,0,0.9)] flex flex-col items-center justify-between overflow-hidden ${
...
function LuxuryGemTile({ type }: { type: "ruby" | "emerald" | "sapphire" }) {
  ...
  return (
    <div className="relative size-full rounded-xl border-[2px] border-[#ca8a04] bg-gradient-to-b from-[#fef08a] via-[#ca8a04] to-[#451a03] p-1 shadow-[inset_0_2px_4px_rgba(255,255,255,0.8),0_4px_8px_rgba(0,0,0,0.85)] flex items-center justify-center">
      <div className="relative size-[88%] rounded-lg border border-[#fbbf24] bg-gradient-to-b from-[#3d1502] to-[#120501] p-1 flex items-center justify-center shadow-inner">
        <div
          className={`size-[82%] ${
...
function AztecMultiplierBadge({ val }: { val: string | number }) {
  if (val === "WHEEL") {
    return (
      <div className="relative size-12 rounded-full border-2 border-yellow-200 bg-gradient-to-tr from-amber-500 via-rose-600 to-yellow-300 shadow-[0_0_15px_#facc15] flex items-center justify-center animate-pulse">
...
function AztecWheelDisc({ rotation }: { rotation: number }) {
  ...
  return (
    <div
      className="relative size-56 rounded-full border-[6px] border-[#fbbf24] shadow-[0_0_25px_#f59e0b,inset_0_0_15px_rgba(0,0,0,0.8)] overflow-hidden flex items-center justify-center transition-transform duration-[1700ms] ease-out"
      style={{
  ...
  const spin = () => {
    ...
    setTimeout(() => {
      ...
      if (willHit) {
        ...
        setWinActive(true);
        playSfx("win");
        toast.success(`🎉 Aztec Hit! +₹${payout}`);
      }
    }, 1750);
  };
  ...
  return (
    <div className="relative mx-auto max-w-[360px] overflow-hidden rounded-3xl border-4 border-[#854d0e] bg-[#0c0501] shadow-2xl font-sans select-none text-slate-100">
      ...
      <div className="relative px-3 pt-2 pb-2 bg-gradient-to-b from-[#381604] via-[#1a0b02] to-[#0a0300]">
        ...
        <div className="relative h-[216px] overflow-hidden rounded-2xl border-4 border-[#b45309] bg-[#120601] p-1 shadow-[inset_0_4px_16px_rgba(0,0,0,1)]">
          ...
          <div className="grid grid-cols-4 gap-1 h-full">
            ...
            {/* Reels 1, 2, 3 Physical Strip Columns */}
                      ...
                      <div key={idx} className="h-[72px] w-full p-1 flex items-center justify-center shrink-0">
                        ...
                        {["A", "K"].includes(sym) && (
                          <div className="size-full rounded-xl border border-amber-950 bg-[#160601] flex items-center justify-center shadow-inner">
                            <span className="font-display text-3xl font-black text-transparent bg-clip-text bg-gradient-to-b from-[#fef08a] via-[#eab308] to-[#92400e] drop-shadow">
                              {sym}
                            </span>
                          </div>
                        )}
        ...
        {winActive && (
          <div
            ...
            className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/85 backdrop-blur-sm cursor-pointer animate-in zoom-in-95 duration-200"
          >
            <div className="relative flex flex-col items-center justify-center">
              <div className="size-20 mb-2">
                <GarudaMaskTile isHit />
              </div>
              <h3 className="font-display text-4xl font-black italic text-transparent bg-clip-text bg-gradient-to-b from-yellow-200 via-amber-400 to-amber-600 drop-shadow">
                BIG WIN!
              </h3>
